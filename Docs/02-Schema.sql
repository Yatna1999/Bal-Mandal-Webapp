...
create index on sabhas (vistar_id) where is_active;
...
create index on karyakars (vistar_id, role) where is_active;

create table karyakar_sabhas (
...
create index on balako (vistar_id) where status = 'active';
create index balako_search_idx on balako using gin (to_tsvector('simple', coalesce(search_blob,'')));
...
create unique index balak_one_primary on balak_sabhas (balak_id) where is_primary;
...
create index on sabha_sessions (session_date, status);
create index on sabha_sessions (sabha_id, session_date desc);

-- karyakars who performed the pre-sabha follow-up for this session (multi-select at sheet footer)
create table session_followup_karyakars (
...
create index on attendance (balak_id, updated_at desc);

-- ============================================================
-- 8. AHNIK
-- ============================================================

create table ahnik_items (
  id uuid primary key default gen_random_uuid(),
...
create index on ahnik_weeks (week_start_date desc);

create table ahnik_entries (
...
create table niyams (
  ...
  end_date date generated always as (start_date + (duration_months || ' months')::interval)::date stored,
...
create index on niyams (balak_id, status);
create index on niyams (end_date) where status = 'active';
...
create index on tasks (status, opens_at);
create index on tasks (sabha_id, status);
...
create index on push_subscriptions (karyakar_id);
...
create index on inapp_notifications (karyakar_id, read_at nulls first, created_at desc);
...
language sql stable security definer set search_path = public as $$
  select case
    when is_vistar_scope()
      then (select array_agg(s.id) from sabhas s where s.vistar_id = auth_vistar())
      else (select array_agg(ks.sabha_id) from karyakar_sabhas ks where ks.karyakar_id = auth.uid())
  end::uuid[] as arr
  cross join lateral unnest(arr) as u(id)
$$;
...
create policy ref_read on standards   for select using (auth.uid() is not null);
create policy ref_read on ahnik_items for select using (auth.uid() is not null);
create policy ref_read on cities      for select using (auth.uid() is not null);
create policy ref_read on zones       for select using (auth.uid() is not null);
create policy ref_read on xetras      for select using (auth.uid() is not null);
create policy ref_read on vistars     for select using (auth.uid() is not null);
...
create view v_karyakar_accountability as
...
create view v_consecutive_absent as
with marked as (
...
with c as (insert into cities (name_gu, name_en) values ('અમદાવાદ','Ahmedabad') returning id),
     z as (insert into zones (city_id, name_gu, name_en) select id,'ઝોન-૧','Zone-1' from c returning id),
     x as (insert into xetras (zone_id, name_gu, name_en) select id,'ક્ષેત્ર-૧૪','Xetra-14' from z returning id),
     v as (insert into vistars (xetra_id, name_gu, name_en) select id,'પાલડી','Paldi' from x returning id)
insert into sabhas (vistar_id, name_gu, name_en, sabha_type, default_weekday, default_start_time, default_end_time)
select v.id, t.gu, t.en, t.typ, t.wd, t.st, t.et
from v, (values
  ('પાલડી બાળ સભા','Paldi Bal Sabha','pakki'::sabha_type_t,3,'21:00'::time,'22:30'::time),
  ('પાલડી શિશુ સભા','Paldi Shishu Sabha','pakki'::sabha_type_t,3,'21:00'::time,'22:30'::time),
  ('સ્વામિનારાયણ પાર્ક-૧ સભા','Swaminarayan Park-1 Sabha','kachi'::sabha_type_t,6,'19:00'::time,'20:00'::time),
  ('રિવર સાઇડ પાર્ક સભા','River Side Park Sabha','kachi'::sabha_type_t,4,'19:00'::time,'20:00'::time)
) as t(gu,en,typ,wd,st,et);
